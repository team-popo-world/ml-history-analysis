...
def avg_buy_ratio(df):
    ...
    buy_sell_counts = transcation_df.groupby(['investSessionId', 'riskLevel'])[['is_buy']].sum().reset_index()
...
def avg_sell_ratio(df):
    ...
    # 각 investSessionId, riskLevel 별로 buy/sell 개수 집계
    buy_sell_counts = transcation_df.groupby(['investSessionId', 'riskLevel'])[['is_sell']].sum().reset_index()
...
def avg_trade_ratio(df, buy, sell):
    transcation_df = df[['investSessionId',
                        'userId',
                        'age',
                        'turn',
    ...
    # merge 두 pivot
    avgTradeRatio = pd.merge(buy, sell, on='investSessionId')
